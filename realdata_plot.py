...
for n in sample_size:
    p = 'nonlinNEW_r50_s2_n{}_conf1.pickle'.format(n)
    ...
    df_n = pd.DataFrame({mae_str: np.concatenate([mae_full, mae_reg, mae_reg_obs, mae_best])[:, 0],
                         ...
                         'Method': np.concatenate([np.repeat('ANM', n_rep), np.repeat('REG', n_rep),
                                                   np.repeat('REG_IND', n_rep), np.repeat('ORACLE', 1)])})
...
df_mae = df_mae[df_mae.Method != 'REG_IND']

# ax = sns.catplot(x='Sample Size', y='Value', hue='Method', col='Quantity',
#             data=df_mae, kind="box",height=4, aspect=.7, sharey=False)
g = sns.FacetGrid(df_mae, hue='Method', height=2.8, aspect=1.15, col='Metric', sharey=False,
                  hue_kws={"marker": ["o", "s", ""]})
...
plt.tight_layout()
plt.savefig("_plots/realdataNEW_nonlin.pdf")

# VARY CONFOUNDING LEVEL
max_corre_list = [0.1, 0.35, 0.6]
df_mae = pd.DataFrame(columns=['Value', 'Method', 'Sample Size', 'Metric'])
for c in max_corre_list:
    p = 'nonlinNEW_r50_s2_n1600_conf{}.pickle'.format(c)
    mae_full, mae_reg, mae_reg_obs, mae_best, _, _, _, SCM = pickle.load(open(os.path.join(dir, p), 'rb'))
    mae_best = mae_best[np.newaxis, :]
    n_rep = len(mae_full)
    df_n = pd.DataFrame({mae_str: np.concatenate([mae_full, mae_reg, mae_reg_obs, mae_best])[:, 0],
...
plt.legend(loc='lower left',
           fancybox=True, title="Method")
plt.tight_layout()
plt.savefig("_plots/realdataNEW_conf.pdf")